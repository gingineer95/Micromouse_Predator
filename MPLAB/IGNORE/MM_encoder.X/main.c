...
//    LCD_clearScreen(0x0000);
    ...
    // Variables 
    unsigned char write_addr = 0b01101100;
    unsigned char read_addr = 0b01101101; // the three address pins are connected to gnd therefore low
    
    // REGISTER ADDRESSES (see data sheet p.12)
    unsigned char ABN = 0x09;
    unsigned char RAW_ANGLE_LOW = 0x0D;
    unsigned char RAW_ANGLE_HIGH = 0x0C;
    unsigned char ZPOS_LOW = 0x01;
    unsigned char ZPOS_HIGH = 0x02;
    unsigned char CONF_LOW = 0x08; // 4 bits total
    unsigned char CONF_HIGH = 0x07; // 6 bits total
    unsigned char PUSHTHR = 0x0A;

    // Configure the ABN register to update at 61Hz
    writePin(write_addr, ABN, 0b0000);
    
    // Read out the actual RAW ANGLE
    val = readPin(write_addr, read_addr, RAW_ANGLE_LOW);
    val = readPin(write_addr, read_addr, RAW_ANGLE_HIGH);
    
    // Write compensation value to ZPOS
    writePin(write_addr, ZPOS_LOW, 0b00000000);
    writePin(write_addr, ZPOS_HIGH, 0b0000);
    
    // Write to CONF and PUSHTHR
    writePin(write_addr, CONF_LOW, 0b0000);
    writePin(write_addr, CONF_HIGH, 0b000000);
    writePin(write_addr, PUSHTHR, 0b00000000);
    ...
    while(1){
        ...
        if (PORTBbits.RB4 == 0){
            int j;
            for (j = 0; j<6 ; j++){                  // run this block 4 times
...
unsigned char readPin(unsigned char write_addr, unsigned char read_addr, unsigned char reg){
    ...
    char m[100];
    sprintf(m, "For register 0x%X, read 0x%x\r\n", reg, rvalue);
